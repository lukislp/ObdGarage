...
/// One SQLite file per deployment (dataDir/carapp.db) - replaces the earlier one-JSON-
/// file-per-entity-type storage. <see cref="EfSyncRepository{T}"/>/<see cref="EfObdSampleStore"/>
/// are the only callers; everything above <see cref="CarApp.Core.IRepository{T}"/> is unaffected.
/// </summary>
public sealed class CarAppDbContext(DbContextOptions<CarAppDbContext> options) : DbContext(options)
{
