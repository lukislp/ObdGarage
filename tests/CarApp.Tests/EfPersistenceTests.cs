...
/// SQLitePCLRaw.bundle_e_sqlite3 3.0.5 override (CarApp.Data.csproj - the NuGet-default 2.1.11
/// has a known advisory) actually works, not just compiles.
/// </summary>
public class EfPersistenceTests
{
    private static IDbContextFactory<CarAppDbContext> NewFactory(string dbPath) => new SqliteDbContextFactory(dbPath);

    private static async Task<string> NewMigratedDbAsync()
    {
        var dbPath = Path.Combine(Path.GetTempPath(), "carapp-ef-" + Guid.NewGuid().ToString("N") + ".db");
    ...
    {
        var dir = Path.Combine(Path.GetTempPath(), "carapp-import-" + Guid.NewGuid().ToString("N"));
