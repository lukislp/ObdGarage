...
/// (src/ObdGarage.Data/JsonlObdSampleStore.cs, src/ObdGarage.Web/Services/SyncManager.cs), all fixed.
/// </summary>
public class JsonlObdSampleStoreBugTests
{
    ...
    {
        var dir = Path.Combine(Path.GetTempPath(), "obdgarage-jsonl-" + Guid.NewGuid().ToString("N"));
...
public class SyncManagerBugTests
{
    ...
    {
        var dbFactory = new SqliteDbContextFactory(Path.Combine(dataDir, "obdgarage.db"));
    ...
    {
        var dir = Path.Combine(Path.GetTempPath(), "obdgarage-sync-logout-" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(dir);
        try
        {
    ...
    {
        var dir = Path.Combine(Path.GetTempPath(), "obdgarage-sync-auth-" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(dir);
        try
        {
            var authPath = Path.Combine(dir, "sync-auth.json");
