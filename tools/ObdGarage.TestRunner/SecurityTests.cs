...
using ObdGarage.Server;
using Microsoft.AspNetCore.Builder;
using Microsoft.Data.Sqlite;
using Microsoft.EntityFrameworkCore;

namespace ObdGarage.TestRunner;
...
///
///  2) SyncService.PushAsync (ObdGarage.Application) used to mark EVERY locally-pending
...
public static class SecurityTests
{
    ...
    private static async Task UserStoreTimingSideChannel(Action<string, bool> check)
    {
        ...
        var dir = Path.Combine(Path.GetTempPath(), "obdgarage-userstore-timing-" + Guid.NewGuid().ToString("N"));
    ...
    private static async Task SyncPushRetriesRejectedEntityInsteadOfLosingItAsync(Action<string, bool> check)
    {
        ...
        var serverDir = Path.Combine(Path.GetTempPath(), "obdgarage-server-sec-" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(serverDir);
        var clientDir = Path.Combine(Path.GetTempPath(), "obdgarage-client-sec-" + Guid.NewGuid().ToString("N"));
        ...
        try
        {
            var dbFactory = new SqliteDbContextFactory(Path.Combine(clientDir, "obdgarage.db"));
            ...
            var clientDir2 = Path.Combine(Path.GetTempPath(), "obdgarage-client-sec2-" + Guid.NewGuid().ToString("N"));
            Directory.CreateDirectory(clientDir2);
            var dbFactory2 = new SqliteDbContextFactory(Path.Combine(clientDir2, "obdgarage.db"));
