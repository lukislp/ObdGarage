...
public static class SyncTests
{
    ...
    {
        var dir = Path.Combine(Path.GetTempPath(), "obdgarage-sync-" + Guid.NewGuid().ToString("N"));
    ...
    public static async Task RunAsync(Action<string, bool> check)
    {
        ...
        var serverDir = Path.Combine(Path.GetTempPath(), "obdgarage-server-" + Guid.NewGuid().ToString("N"));
