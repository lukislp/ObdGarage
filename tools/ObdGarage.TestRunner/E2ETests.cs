...
public static class E2ETests
{
    ...
    {
        var dir = Path.Combine(Path.GetTempPath(), "obdgarage-e2e-" + Guid.NewGuid().ToString("N"));
        ...
        var dbPath = Path.Combine(dir, "obdgarage.db");
        ...
        // fails with "file in use" since the pooled handle to obdgarage.db is still open.
